...
dVec series_cp_lik(
    ...
    /*
     * Computes the likelihood of a series being generated by a given rate, 
     *  with or without change-point. 
     */
    ...
    for (int i = 0; i <= (n - ws); i++) {
      ...
      lik[i] = 1.0;
      // ... series values in entire window
      ...
      if (null) {
        // Product of likelihoods over entire window
        ...
        // Product of likelihoods over two halves of window
...
IntegerVector LROcp(
    ...
    dVec lik_null = series_cp_lik(series, ws, filter_ws, true);
    
    // Compute the change-point negative log-likelihood at each bin
    dVec lik_cp = series_cp_lik(series, ws, filter_ws, false);
...
IntegerMatrix LROcp_array(
    ...
    for (int s = 0; s < n_series; s++) {
      ...
      // Find the likelihood ratio at each sample
    ...
    for (int s = 0; s < n_series; s++) {
      lik_ratio_array.column(s) = Rcpp::log(lik_ratio_array.column(s));
      //lik_ratio_array.column(s) = to_NumVec(roll_mean(to_dVec(lik_ratio_array.column(s)), filter_ws));
    }
    
    // Use LROcp on this series to estimate change points 
    // ... returned values are the zero-base indices of the change points in the centroid
    // ... add 1 to get the one-base indices, as next R function expects
    IntegerVector found_cp = LROcp(to_dVec(centroid), ws, filter_ws, out_mult) + 1;
